...
const INITIAL_CATEGORIES = [
    { name: 'Food & Dining', color: '#0088FE', icon: 'food' },
    { name: 'Transportation', color: '#00C49F', icon: 'transport' },
    { name: 'Shopping', color: '#FFBB28', icon: 'shopping' },
    { name: 'Utilities', color: '#FF8042', icon: 'utilities' },
    { name: 'Entertainment', color: '#AF19FF', icon: 'entertainment' },
    { name: 'Housing', color: '#FF1943', icon: 'housing' },
    { name: 'Health', color: '#19D7FF', icon: 'health' },
    { name: 'Other', color: '#6c757d', icon: 'other' },
];
...
function App() {
  ...
  const processPdf = useCallback(async (file: File) => {
    ...
    try {
      ...
      const result = await analyzeExpensePdf(file, categoryNames);
       // Add a blank notes field to each transaction for the UI
      const resultWithNotes = {
        ...result,
        creditTransactions: result.creditTransactions.map((tx: any) => ({ ...tx, notes: '' })),
        debitTransactions: result.debitTransactions.map((tx: any) => ({ ...tx, notes: '' })),
      };
      setSummary(resultWithNotes);
    } catch (err) {
  ...
  const handleUpdateNotes = (type: 'credit' | 'debit', transactionIndex: number, newNotes: string) => {
    if (!summary) return;
    ...
    if (!transaction || transaction.notes === newNotes) return;

    transaction.notes = newNotes;
    setSummary(newSummary);

    if (modal) {
      setModal(prevModal => prevModal ? { ...prevModal, transactions: transactions } : null);
    }
  };
  ...
  const handleAddScannedTransaction = (newTx: { description: string, date: string, amount: number, category: string }) => {
    ...
    const newSummary = JSON.parse(JSON.stringify(summary));
    const newTxWithNotes = { ...newTx, notes: '' };
    
    newSummary.debitTransactions.unshift(newTxWithNotes);
  ...
  const handleExportAllXLSX = () => {
      ...
      const creditData = summary.creditTransactions.map((tx: any) => ({ Date: tx.date, Description: tx.description, Amount: tx.amount, Notes: tx.notes || '' }));
      ...
      const debitData = summary.debitTransactions.map((tx: any) => ({ Date: tx.date, Description: tx.description, Amount: tx.amount, Category: tx.category, Notes: tx.notes || '' }));
  ...
  const handleExportAllPDF = () => {
    ...
    // Credit Transactions
    doc.setFontSize(12);
    doc.text("Credit Transactions", 14, 22);
    const creditBody = summary.creditTransactions.map((tx: any) => [tx.date, tx.description, tx.amount.toFixed(2), tx.notes || '']);
    ...
    // Debit Transactions - The robust way
    const lastTable = (doc as any).autoTable.previous;
    let debitTableStartY = 25; // A safe default.
    if (lastTable && lastTable.finalY) {
        debitTableStartY = lastTable.finalY + 15;
    }

    doc.setFontSize(12);
    doc.text("Debit Transactions", 14, debitTableStartY - 4);
    const debitBody = summary.debitTransactions.map((tx: any) => [tx.date, tx.description, tx.amount.toFixed(2), tx.category, tx.notes || '']);
    (doc as any).autoTable({
        head: [['Date', 'Description', 'Amount', 'Category', 'Notes']],
        body: debitBody,
        startY: debitTableStartY,
        theme: 'grid',
    });
